...
export async function interviewStep(
  ...
  const system = [
    ...
    `- When you ask WHERE the data lives, only offer sources Loopsmith can actually ingest: Slack, Gmail/email, Notion, Google Calendar, customer calls (Fathom), Stripe — plus an "I'll paste it in" option. NEVER offer spreadsheets, Google Drive, or other apps we can't connect; if their data lives somewhere unsupported, map it to "paste it in".`,
    ...
    `- sensors: lowercase source keys ONLY from {slack, gmail, fathom, calendar, notion, stripe} (the connectors that actually exist) that fit their answer. If they'll paste data in, still pick the closest key(s) from that set.`,
